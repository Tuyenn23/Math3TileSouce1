...
public class CustomPostBuild
{
	...
	public static void OnPostprocessBuild(BuildTarget buildTarget, string pathToXcode)
	{
		if (buildTarget == BuildTarget.iOS)
		{
			...
			pbxProject.SetBuildProperty(target, "ENABLE_BITCODE", VipEditorSetting.Instance.IOS_Build_enableBitcode ? "YES" : "NO");
			Debug.Log("Bitcode " + VipEditorSetting.Instance.IOS_Build_enableBitcode);
			if (VipEditorSetting.Instance.CapabilityTypeList.Length > 0)
			{
				foreach (var item in VipEditorSetting.Instance.CapabilityTypeList)
				{
					string itemString = "";
					switch (item)
					{
	...
	static void AddPListValues(string pathToXcode)
	{
		...
		if (VipEditorSetting.Instance.IOS_Build_NSUserTrackingUsageDescriptions)
		{
			Debug.Log("NSUserTrackingUsageDescriptions on");
			plistRoot.SetString("NSUserTrackingUsageDescription", TrackingDescription);
		}
		if (VipEditorSetting.Instance.IOS_Build_NSCalendarsUsageDescription)
		{
		...
		if (VipEditorSetting.Instance.IOS_Build_SKAdNetworkItems)
		{
